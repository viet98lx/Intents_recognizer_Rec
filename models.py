...
class Beacon(Model):
    ...
    def __init__(self, sess, emb_dim, rnn_units, alpha,
        ...
        with tf.variable_scope(self.scope):
            ...
            self.I_B = tf.get_variable(dtype=tf.float32, initializer=tf.constant(uniform_initializer, dtype=tf.float32),
                                       name="I_B")
            ...
            with tf.name_scope("Basket_Sequence_Encoder"):
                self.bseq = tf.sparse_placeholder(shape=(batch_size, self.max_seq_length, self.nb_items),
                                                  dtype=tf.float32, name="bseq_input")
                self.bseq_length = tf.placeholder(dtype=tf.int32, shape=(batch_size,), name='bseq_length')

                self.bseq_dense = tf.sparse.to_dense(self.bseq)
                self.bseq_encoder = tf.reshape(self.bseq_dense, shape=[-1, self.nb_items], name="bseq_2d")
                ...
                rnn_encoder = create_rnn_encoder(self.bseq_encoder, self.rnn_units, rnn_dropout_rate, self.bseq_length,
                                                 rnn_cell_type,
            ...
            with tf.name_scope("Next_Basket"):
                W_H = tf.get_variable(dtype=tf.float32, initializer=tf.initializers.glorot_uniform(),
    ...
    def get_topk_tensor(self, x, k=10):
        ...
        index_rows = tf.ones(shape=(self.batch_size, k), dtype=tf.int32) * tf.expand_dims(tf.range(0, self.batch_size),
                                                                                          axis=-1)
        ...
        sparse_tensor = tf.SparseTensor(indices=top_k_indices, values=top_k_values,
                                        dense_shape=[self.batch_size, self.nb_items])
